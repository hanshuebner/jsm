...
MIDI.MIDIInput.prototype.listen = function()
{
    ...
    console.log('setFilters channelMask ' + this.channelMask.toString(2) + ' filters ' + this.currentFilter.toString(2));
    this.setFilters(this.channelMask, this.currentFilter);

    (function (midiInput) {
        console.log('listening to newListener');
        midiInput.on('newListener', function (type, listener) {
            // Reset the filter bit corresponding to the event type in the MIDI listener
            var bitToReset = filterBits[type] || 0;
            console.log('new listener for ' + type + ', bit is ' + bitToReset.toString(2));
            midiInput.currentFilter &= ~bitToReset;
            console.log('setFilters channelMask ' + midiInput.channelMask.toString(2) + ' filters ' + midiInput.currentFilter.toString(2));
